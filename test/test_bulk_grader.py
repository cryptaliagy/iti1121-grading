...
#300167116,#BJONES002,Jones,Bob,bjones002@example.edu,,#"""
        ...
        assert lines[0].strip() == "OrgDefinedId,Username,Last Name,First Name,Email,Lab Grade,End-of-Line Indicator"
...
300167116,BJONES002,Jones,Bob,bjones002@example.edu,,#"""
        ...
        df, original_header = load_grading_list(csv_file)
        
        # Should filter out rows starting with #
        assert len(df) == 2
        assert "ASMITH001" in df['Username'].values
        assert "BJONES002" in df['Username'].values
        assert "#BJONES002" not in df['Username'].values
        
        # Should preserve original header
        assert "Lab 1 Grade" in original_header
...
class TestNameMatching:
    ...
    @pytest.mark.parametrize("query,candidates,expected", [
        # Exact match
        ("Charlie Wilson", ["Charlie Wilson", "Dana Rodriguez", "Emma Thompson"], "Charlie Wilson"),
        
        # Case insensitive
        ("charlie wilson", ["Charlie Wilson", "Dana Rodriguez", "Emma Thompson"], "Charlie Wilson"),
        
        # With accents - exact match
        ("José María", ["José María", "François Côté", "Dana Rodriguez"], "José María"),
        
        # With accents - without accents input
        ("Jose Maria", ["José María", "François Côté", "Dana Rodriguez"], "José María"),
        
        # No good match
        ("Completely Different Name", ["Charlie Wilson", "Dana Rodriguez", "Emma Thompson"], None),
    ])
    ...
    @pytest.mark.parametrize("query,candidates,threshold,expected", [
        # Partial match above threshold
        ("Charlie", ["Charlie Wilson", "Dana Rodriguez", "Emma Thompson"], 50, "Charlie Wilson"),
        
        # Partial match below threshold
        ("Char", ["Charlie Wilson", "Dana Rodriguez", "Emma Thompson"], 60, None),
        
        # International partial matches
        ("José María", ["José María González", "François Müller", "Björn Åström"], 60, "José María González"),
        ("Dvorak", ["José María González", "François Müller", "Václav Dvořák"], 60, "Václav Dvořák"),
    ])
    def test_find_best_name_match_with_threshold(self, query, candidates, threshold, expected):
    ...
    @pytest.mark.parametrize("query,candidates,expected", [
        ...
        ("José María González", ["José María González", "François Müller", "Björn Åström", "Václav Dvořák"], "José María González"),
        
        # International matches without accents
        ("Jose Maria Gonzalez", ["José María González", "François Müller", "Björn Åström", "Václav Dvořák"], "José María González"),
        ("Francois Muller", ["José María González", "François Müller", "Björn Åström", "Václav Dvořák"], "François Müller"),
        ("Bjorn Astrom", ["José María González", "François Müller", "Björn Åström", "Václav Dvořák"], "Björn Åström"),
        ("Vaclav Dvorak", ["José María González", "François Müller", "Björn Åström", "Václav Dvořák"], "Václav Dvořák"),
        
        # Case variations
        ("JOSÉ MARÍA GONZÁLEZ", ["José María González", "François Müller", "Björn Åström", "Václav Dvořák"], "José María González"),
        ("françois müller", ["José María González", "François Müller", "Björn Åström", "Václav Dvořák"], "François Müller"),
        
        # Hyphenated and compound names
        ("van der berg", ["Van Der Berg", "O'Connor-Smith", "MacLeod", "de la Cruz"], "Van Der Berg"),
        ("macleod", ["Van Der Berg", "O'Connor-Smith", "MacLeod", "de la Cruz"], "MacLeod"),
        ("de la cruz", ["Van Der Berg", "O'Connor-Smith", "MacLeod", "de la Cruz"], "de la Cruz"),
    ])
    ...
    @pytest.mark.parametrize("query,candidates,threshold,expected", [
        ...
        ("O'Connor Smith", ["Van Der Berg", "O'Connor-Smith", "MacLeod", "de la Cruz"], 80, "O'Connor-Smith"),
    ])
    def test_find_best_name_match_compound_with_threshold(self, query, candidates, threshold, expected):
...
class TestSubmissionParsing:
    ...
    def test_parse_submission_folder_name_invalid(self):
        ...
        with pytest.raises(ValueError):
            parse_submission_folder_name("123-456 - Name - InvalidMonth 1, 2025 1200 PM")
...
class TestFileOperations:
    ...
    def test_extract_submissions(self, tmp_path):
        ...
        with zipfile.ZipFile(zip_path, 'w') as zip_file:
            zip_file.writestr("152711-351765 - Charlie Wilson - May 18, 2025 1224 PM/Test.java", "// Java code")
            zip_file.writestr("123456-789012 - Dana Rodriguez - June 15, 2025 930 AM/Test.java", "// Java code")
        ...
        assert (submissions_dir / "152711-351765 - Charlie Wilson - May 18, 2025 1224 PM").exists()
        assert (submissions_dir / "123456-789012 - Dana Rodriguez - June 15, 2025 930 AM").exists()
    ...
    def test_prepare_grading_directory_with_zip_files(self, tmp_path):
        ...
        assert (grading_dir / "Test.java").exists()
        assert (grading_dir / "Helper.java").exists()
        assert not (grading_dir / "README.txt").exists()  # Should only extract Java files
...
class TestGradingWorkflow:
    ...
    def test_save_results_to_csv(self, tmp_path):
        ...
        # Create test results
        student1 = StudentRecord('300069634', 'CWILS001', 'Wilson', 'Charlie', 'cwils001@example.edu')
        student2 = StudentRecord('300173416', 'DRODR002', 'Rodriguez', 'Dana', 'drodr002@example.edu')
        
        results = [
            GradingResult(student1, 0.85, success=True),
            GradingResult(student2, 0.0, error_message="Compilation failed", success=False)
        ]
        
        output_path = tmp_path / "results.csv"
        original_header = "OrgDefinedId,Username,Last Name,First Name,Email,Lab 1 Grade,End-of-Line Indicator"
        
        # Test with failure as 0
        save_results_to_csv(results, original_df, original_header, output_path, failure_is_null=False)
        ...
        save_results_to_csv(results, original_df, original_header, output_path, failure_is_null=True)
        ...
        grade_index = 5  # Lab Grade column
...
class TestDataStructures:
    ...
    def test_grading_result(self):
        ...
        student = StudentRecord("300069634", "CWILS001", "Wilson", "Charlie", "cwils001@example.edu")
